...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    void paymentsService.getAll().then(setRows);
  }, []);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row">
        <SearchInput value={search} onChange={setSearch} placeholder="Search transactions…" className="sm:max-w-xs" />
        <Select
          options={[
      ...
      <Table
        data={filtered}
        rowKey={(r) => r.id}
        empty={<EmptyState title="No transactions" description="Adjust filters to see payment activity." />}
        columns={[
